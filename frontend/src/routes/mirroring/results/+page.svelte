<script>
  ...
  onMount(() => {
    ...
    // Read ONLY the mirroring keys
    score   = Number(localStorage.getItem('mirroring_score')  || 0);
    total   = Number(localStorage.getItem('mirroring_total')  || 0);
    results = readJSON('mirroring_results', []);

    // Back-compat fallback (old generic keys)
    if (!results.length && localStorage.getItem('quiz_results')) {
      results = readJSON('quiz_results', []);
      score   = Number(localStorage.getItem('quiz_score')  || score);
      total   = Number(localStorage.getItem('quiz_total')  || total);
    }
  ...
  function clearAndRestart() {
    localStorage.removeItem('mirroring_results');
    localStorage.removeItem('mirroring_score');
    localStorage.removeItem('mirroring_total');
...
<style>
  ...
  .result-box {
    ...
    margin:auto; box-shadow:0 4px 30px rgba(0,0,0,0.2); position:relative;
  }

  .title {
    font-family: Georgia, serif; font-size: 2.5rem; margin: 0 0 8px;
  ...
  .btn {
    ...
    font-size:20px; font-weight:bold; color:#000; background:#fff;
