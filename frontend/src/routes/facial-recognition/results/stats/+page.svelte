<script>
  ...
  onMount(() => {
    ...
    // graceful fallback if somehow only booleans exist
    if (!details.length) {
  ...
  function playAgain() {
    // optional: keep quiz_details around if you want users to review later
...
<style>
  ...
  .glass {
    ...
    max-height: 84vh;             /* <-- keeps inside the viewport */
    overflow: hidden;             /* child list will scroll */
  }
  ...
  .dot {
    width: 36px; height: 8px; border-radius: 4px; background: #ddd;
  }
...
<!-- Background blobs (same set you use everywhere) -->
...
<div class="page">
  <div class="glass">
    ...
    <div class="list">
      {#each details as d, i}
        <div class="row">
          <!-- thumbnail (falls back to empty box if missing) -->
          {#if d.img}
