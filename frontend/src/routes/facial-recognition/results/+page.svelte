...
<script lang="ts">
  ...
  onMount(() => {
    ...
    score = Number(localStorage.getItem('fr_score') ?? localStorage.getItem('quiz_score') ?? 0);
    total = Number(localStorage.getItem('fr_total') ?? localStorage.getItem('quiz_total') ?? 0);

    let raw: any[] = [];
    try {
      raw = JSON.parse(localStorage.getItem('fr_results') ?? localStorage.getItem('quiz_results') ?? '[]');
  ...
  function playAgain() {
    localStorage.removeItem('fr_results');
    localStorage.removeItem('fr_score');
    localStorage.removeItem('fr_total');
