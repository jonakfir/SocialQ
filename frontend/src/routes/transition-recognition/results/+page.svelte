<script>
  ...
  onMount(() => {
    ...
    score = Number(localStorage.getItem(`tr_quiz_score_${userKey}`) || 0);
    total = Number(localStorage.getItem(`tr_quiz_total_${userKey}`) || 0);

    let raw = [];
    try { raw = JSON.parse(localStorage.getItem(`tr_quiz_results_${userKey}`) || '[]'); }
    catch { raw = []; }
  ...
  function clearAndRestart() {
    ...
    localStorage.removeItem(`tr_quiz_results_${userKey}`);
    localStorage.removeItem(`tr_quiz_score_${userKey}`);
