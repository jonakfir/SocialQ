<script lang="ts">
  ...
  let scoring = false;
  let frozen = false;            // if true, we stop drawing new frames to the canvas (video keeps playing)
  let errorMsg = '';
  let cameraError = '';
  let statusMsg = '';
  ...
  // tuning
  const PASS_THRESHOLD = 0.45;  // friendlier threshold after multi-frame averaging
  const APP_LINK = 'https://social-q-theta.vercel.app/';

  // ── Camera: start immediately; never pause/stop stream ────
  ...
  // ── Emotion detection: multi-frame average ─────────────────
  async function detectEmotionAvg(frames = 8, gapMs = 50) {
    ...
    for (let i = 0; i < frames; i++) {
      drawFrameToCanvas();                 // always judge what the user sees
  ...
  // quick brightness check -> 0(dark) .. 255(bright)
  ...
  async function shareImage() {
    ...
    try {
      if (!lastShotDataUrl) {
        ...
        showToast('Message copied — paste in Messages');
        closeAllCards();
        return;
      }
      ...
      if (copied) showToast('Photo copied — paste it in Messages');
        ...
        try { await navigator.clipboard?.writeText?.(text); } catch {}
        showToast('Couldn’t copy photo — message text is copied');
      }
    } catch {
      try { await navigator.clipboard?.writeText?.(`Try AboutFace: ${APP_LINK}`); } catch {}
      showToast('Sharing unavailable — link copied');
    } finally {
      // close popup and keep camera running (canvas will resume drawing)
      closeAllCards();
    }
  }

  // ── Capture & check (freeze canvas only) ───────────────────
  async function captureAndCheck() {
    ...
    try {
      ...
      // Sample multiple frames over ~400ms for stability
      const { probs, top } = await detectEmotionAvg(8, 50);

      // Freeze the exact frame we’ll show/share
      drawFrameToCanvas();
      lastShotDataUrl = dataURL();
      frozen = true; // IMPORTANT: do not pause/stop the video stream
      ...
      if (pass) {
        ...
        // Optional helpful hint without failing:
        const b = estimateBrightness(canvasEl);
        if (b < 70) {
          // showToast('Tip: increase lighting for even better results');
        }
      } else {
        const b = estimateBrightness(canvasEl);
      ...
      // canvas is frozen; user hits Try again to resume drawing
    } finally {
      scoring = false;
    }
  }
  ...
  function resetShot() {
    errorMsg = '';
    showPassFrame = false;
    frozen = false;            // resume live drawing immediately
    if (frameTimer) clearTimeout(frameTimer);
    frameTimer = null;
  }
  ...
  onMount(async () => {
    ...
    // Continuous render loop: draws live feed unless frozen
    (function loop(){
...
<svelte:head><title>Collage – SocialQ</title></svelte:head>

<!-- background blobs (served by /style.css) -->
...
<div class="panel" bind:this={panelEl}>
  <!-- Give video real size so browsers negotiate dimensions reliably (hidden via opacity) -->
...
<style>
  @import '/style.css'; /* static/style.css */
  ...
  /* Take another = black text on white with black border */
