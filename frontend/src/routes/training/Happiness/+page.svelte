<script lang="ts">
  ...
  // ------- page config -------
  const EMOTION = 'Happy';
  const QUOTE = '“Happiness depends upon ourselves.”';
  const QUOTE_ATTR = '— Aristotle';
  ...
  type TipKey = 'brows' | 'eyes' | 'cheeks' | 'mouth';
  ...
  const tips: Array<{
    ...
    { key: 'brows', title: 'Brows', text: 'Slightly raised and curved, relaxed.', x: '95%', y: '37%', label: 'Brows' },
    { key: 'eyes',  title: 'Eyes',  text: '“Crow’s feet” wrinkling can be seen at outer corners of eyes.',   x: '95%', y: '43%', label: 'Eyes'  },
    { key: 'cheeks',  title: 'Cheeks',  text: 'Cheeks are raised with the cheekbone muscles pushing up on the eyes.',   x: '95%', y: '53%', label: 'Cheeks'  },
    { key: 'mouth', title: 'Mouth', text: 'corners drawn back and slightly up, lips parted or slightly pressed.',     x: '95%', y: '63%', label: 'Mouth' }
  ];
    ...
    {#if hoverTip && !openTip}
      <div class="hover-panel">
        ...
        <div class="hover-card hover-white">
          ...
          <div class="hover-text">Click a helper (Brows, Eyes, Cheeks or Mouth) to open the tip for that area.</div>
        </div>
      </div>
    {/if}
