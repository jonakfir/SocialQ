<script lang="ts">
  ...
  import DisgustClip from '$lib/assets/ekman/DisgustPractice.mp4';

  // ------- page config -------
  const EMOTION = 'Disgust';
  const QUOTE = '“Disgust is a powerful emotion, often more influential than reason.”';
  const QUOTE_ATTR = '— Sigmund Freud';
  ...
  type TipKey = 'brows' | 'eyes' | 'nose' | 'mouth';
  ...
  const tips: Array<{
    ...
    { key: 'brows', title: 'Brows', text: 'Eyebrows are lowered and tightened, often pushing inward.', x: '95%', y: '38%', label: 'Brows' },
    { key: 'eyes',  title: 'Eyes',  text: 'Eyes are slightly closed or looking away, scrunched up.',   x: '95%', y: '42%', label: 'Eyes'  },
    { key: 'nose',  title: 'Nose',  text: 'Nose is scrunched up and pulled away from the scene in front.',   x: '95%', y: '52%', label: 'Nose'  },
    { key: 'mouth', title: 'Mouth', text: 'Lower lip raised and slightly forward, pushed toward upper lip or slightly protruding.',     x: '95%', y: '61%', label: 'Mouth' }
  ];
    ...
    {#if hoverTip && !openTip}
      <div class="hover-panel">
        ...
        <div class="hover-card hover-white">
          ...
          <div class="hover-text">Click a helper (Brows, Eyes, Nose or Mouth) to open the tip for that area.</div>
        </div>
      </div>
    {/if}
