<script lang="ts">
  ...
  // ------- page config -------
  const EMOTION = 'Surprise';
  const QUOTE =
    '“Life is a succession of lessons which must be lived to be understood. And often the greatest lessons come as surprises.”';
  const QUOTE_ATTR = '— Ralph Waldo Emerson';
  ...
  // Tweak per page if you want different copy:
  ...
  // ---- SHOW-ONCE logic with a "force" override  --------------------------------
  const SHOW_ONCE = true;

  // CHANGE 1: version your key so you can re-show for everyone by bumping COACH_VER
  const COACH_VER = 'v2'; // <— bump this to re-show globally after a change
  const COACH_KEY =
    typeof window !== 'undefined'
      ? `coach_seen:${location.pathname}:${COACH_VER}`
      : 'coach_seen';

  onMount(() => {
    // CHANGE 2: force override via ?coach in the URL (handy for demos/QA)
    const force =
      typeof window !== 'undefined' &&
    ...
    if (force) {
      coachOpen = true;
      return; // skip the "seen" check below
    }

    // If we've shown this (versioned) coach on this route, auto-close it
    try {
  ...
  function advanceCoach() {
    if (coachStep < coachSteps.length - 1) {
      ...
      coachOpen = false;
      try {
        if (SHOW_ONCE) localStorage.setItem(COACH_KEY, '1');
      } catch {}
    }
  }
  // ------------------------------------------------------------------------------
  ...
  type TipKey = 'forehead' | 'brows' | 'eyes' | 'mouth';
  ...
  const tips: Array<{
  ...
  }> = [
    {
      key: 'forehead',
      title: 'Forehead',
      text: 'Forehead is tense and scrunched with wrinkles pronounced',
      x: '95%',
      y: '28%',
      label: 'Forehead'
    },
    {
      key: 'brows',
      title: 'Brows',
      text: 'Surprised eyebrows appear curved and high',
      x: '95%',
      y: '33%',
      label: 'Brows'
    },
    {
      key: 'eyes',
      title: 'Eyes',
      text: 'Eyes open wide during surprise, ‘whites’ sometimes visible',
      x: '95%',
      y: '38%',
      label: 'Eyes'
    },
    {
      key: 'mouth',
      title: 'Mouth',
      text: 'Jaw drops during surprise, lips and teeth part',
      x: '95%',
      y: '61%',
      label: 'Mouth'
    }
  ...
  onMount(async () => {
    ...
    (async function drawLoop() {
      ...
      if (showOverlay && lastResult) {
        try {
          human.draw.all(canvas, lastResult);
        } catch {}
      }
...
<!-- CHANGE 3: while the coach is open, mark the main UI aria-hidden -->
<div class="container" aria-hidden={coachOpen}>
  <!-- LEFT: live camera -->
  <section class="pane left">
    ...
    <div class="left-controls">
      <button class="chip" on:click={() => (showOverlay = !showOverlay)}>
      ...
      <div class="hover-panel">
        ...
        <div class="hover-card hover-white">
          ...
          <div class="hover-text">
            Click a helper (Forehead, Brows, Eyes, or Mouth) to open the tip for that area.
          </div>
        </div>
      </div>
    {/if}
...
<style>
  ...
  .container {
    ...
    background:
      linear-gradient(180deg, rgba(255, 255, 255, 0.3), rgba(255, 255, 255, 0.22)),
      radial-gradient(1200px 800px at 10% 0%, rgba(79, 70, 229, 0.12), transparent 60%),
      radial-gradient(1200px 800px at 90% 20%, rgba(34, 211, 238, 0.12), transparent 60%);
  ...
  .clip-frame {
    position: absolute;
    inset: 16px 16px calc(16px + var(--quote-space)) 16px;
    border: 2px solid #111;
    border-radius: 14px;
    background: transparent;
  ...
  .hover-card {
    ...
    border-radius: 14px;
    padding: 10px 14px;
    box-shadow: 0 14px 34px rgba(234, 179, 8, 0.35);
  ...
  .strip-mask::before {
    top: 0;
    height: calc((var(--hole-y)) - var(--half));
    border-top-left-radius: 16px;
    border-top-right-radius: 16px;
  }
  .strip-mask::after {
    top: calc((var(--hole-y)) + var(--half));
    bottom: 0;
  ...
  .quote-bar {
    ...
    right: 0;
    bottom: 0;
    height: var(--quote-space);
    display: flex;
    align-items: center;
    justify-content: center;
    z-index: 40;
    background: linear-gradient(0deg, rgba(255, 255, 255, 0.72), rgba(255, 255, 255, 0.42));
  ...
  /* --- Coach overlay styles --- */
  .coach-backdrop {
    position: fixed;
    inset: 0;
    z-index: 1000;
    background:
      radial-gradient(60% 40% at 20% 10%, rgba(79, 70, 229, 0.18), transparent 60%),
      radial-gradient(50% 40% at 80% 30%, rgba(34, 211, 238, 0.16), transparent 60%),
      rgba(0, 0, 0, 0.45);
    display: grid;
    place-items: center;
    animation: coachFadeIn 0.18s ease both;
  }
  .coach {
    display: grid;
