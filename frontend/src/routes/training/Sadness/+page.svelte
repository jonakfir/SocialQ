<script lang="ts">
  ...
  import SadClip from '$lib/assets/ekman/SadPractice.mp4';

  // ------- page config -------
  const EMOTION = 'Sad';
  const QUOTE = '“There is a sacredness in tears. They are not the mark of weakness, but of power.”';
  const QUOTE_ATTR = '— Washington Irving';
  ...
  const tips: Array<{
    ...
    { key: 'brows', title: 'Brows', text: 'Inner corners raised up and pulled in together.', x: '95%', y: '38%', label: 'Brows' },
    { key: 'eyes',  title: 'Eyes',  text: 'Soft and often looking down.',   x: '95%', y: '42%', label: 'Eyes'  },
    { key: 'mouth', title: 'Mouth', text: 'The corners of the lips are drawn down, may appear trembling.',     x: '95%', y: '66%', label: 'Mouth' }
  ];
...
<!-- >>> CHANGE: add aria-hidden so the main UI is inert while coach is open -->
