...
<script lang="ts">
  ...
  import { getViewAs, clearViewAs, isViewingAsPersonal, isViewingAsOrgAdmin } from '$lib/viewAs';
  ...
  // Track dashboard route
  $: isDashboard = $page.url.pathname.toLowerCase() === '/dashboard';
  ...
  $: viewAsOrgAdmin = isViewingAsOrgAdmin();
  // Hide menu button on dashboard (show only in view-as modes)
  $: showMenuButton = showMenu && !isDashboard;
</script>
  ...
  {#if showMenuButton}
    <div class="menu-wrap" bind:this={wrapEl}>
      ...
      <div class="menu-pop" data-open={isOpen}>
        ...
        {#if viewAsPersonal}
          <button class="item" on:click={exitPersonalMode}>Exit Personal Mode</button>
        {/if}
        {#if viewAsOrgAdmin}
          <button class="item" on:click={exitOrgAdminMode}>Exit Org Admin Mode</button>
        {/if}
        {#if user}
